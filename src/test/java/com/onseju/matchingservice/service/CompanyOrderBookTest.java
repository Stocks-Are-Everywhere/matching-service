...
public class CompanyOrderBookTest {
	...
	class LimitOrderTests {
		...
		void matchBuyOrderWithSamePriceSellOrder() {
			...
			Collection<MatchedEvent> responses = orderBook.received(buyOrder);
		...
		void matchSellOrderWithSamePriceBuyOrder() {
			...
			Collection<MatchedEvent> responses = orderBook.received(sellOrder);
		...
		void buyOrderImbalance() {
			...
			Collection<MatchedEvent> response = orderBook.received(sellOrder);
		...
		void sellOrderImbalance() {
			...
			Collection<MatchedEvent> response = orderBook.received(buyOrder);
	...
	class MarketOrderTests {
		...
		void receiveMarketBuyOrder() {
			...
			// when
			List<MatchedEvent> responses = new ArrayList<>();
		...
		void receiveMarketSellOrder() {
			...
			// when
			orderBook.received(buyOrder);
			List<MatchedEvent> responses = new ArrayList<>(orderBook.received(marketSellOrder));
		...
		void matchBuyMarketOrderWithLowestPrice() {
			...
			orderBook.received(sellOrder1);
			orderBook.received(sellOrder2);
			List<MatchedEvent> responses = orderBook.received(marketBuyOrder);
		...
		void matchSellMarketOrderWithHighestPrice() {
			...
			orderBook.received(buyOrder1);
			orderBook.received(buyOrder2);
			List<MatchedEvent> responses = orderBook.received(marketSellOrder);

			// Then
			assertThat(responses).hasSize(2);

			// 낮은 가격의 주문 먼저 체결
			MatchedEvent sellOrderEvent2 = responses.get(0);
			...
			MatchedEvent sellOrderEvent1 = responses.get(1);
	...
	class MatchingSequenceTests {
		...
		void buyOrderQuantityPriorityMatching() {
			...
			TradeOrder buyOrder1 = createOrder(1L, Type.LIMIT_BUY, new BigDecimal("50000"), new BigDecimal("5"), 1L,
					createdAt);
			TradeOrder buyOrder2 = createOrder(2L, Type.LIMIT_BUY, new BigDecimal("50000"), new BigDecimal("10"), 1L,
					createdAt);
			TradeOrder sellOrder = createOrder(3L, Type.LIMIT_SELL, new BigDecimal("50000"), new BigDecimal("5"), 2L,
					createdAt);
			...
			orderBook.received(buyOrder1);
			orderBook.received(buyOrder2);
			List<MatchedEvent> results = orderBook.received(sellOrder);
	...
	class MarketPrice {
		...
		void changeOrderStatusToCompleteWhenTradeIsExecuted() {
			// given
			Long accountId = 1L;
			TradeOrder sellOrder = createOrder(1L, Type.LIMIT_SELL, new BigDecimal("50000"), new BigDecimal("5"),
					accountId);
			TradeOrder buyOrder = createOrder(3L, Type.LIMIT_BUY, new BigDecimal("50000"), new BigDecimal("5"),
					accountId);
